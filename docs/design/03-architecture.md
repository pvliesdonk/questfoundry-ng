...
| A21 | A pass whose output is many independent items decomposes into one right-sized call per item, each carrying only that item's context — not one greedy call over the whole set. POLISH's passage layer is the first: `finalize` expands (runner `PassSpec.expand`) into a `summary:<group>` per collapse group + a `labels:<group>` per source group, replacing the single `passages` pass | Raising `num_ctx` to fit the whole-layer call (the model supports 128k); a fixed batch size `B`; keeping one call and accepting the ceiling | The single call's context (every group's beats + every edge) gave *no per-item benefit* — a passage summary derives from its own group's beats alone — yet its ~15–17k-token output overran a weak provider's `num_ctx` at medium scope (a real `AdapterError`) and inflated every call. Call count is not a cost the pipeline optimizes against (FILL already runs ~150 write calls per medium story); independent minimal-context calls are strictly better for weak tiers, and a batch constant is an arbitrary knob with no natural value — the item is the natural unit. Splitting passage *creation* from choice *wiring* forces the variant→flag mapping the single pass held in memory onto the graph (`Passage.variant_flag`), which is also just more honest state |
| A22 | Rotating limited POV is a per-beat `viewpoint` annotation (+ `interlude` mark) written by GROW's *annotate* pass pre-freeze, enforced per **passage**: collapse cuts a boundary at every head-switch (one head per passage, I14), FILL derives each passage's head from its beats (computed, never stored) and keys *no other minds* to it, while `Voice.pov` stays the scheme and `Voice.interlude` the marked deviant register. Headless passages degrade to the book-wide rule (author-confirmed design, 2026-07-14, `docs/plans/rotating-pov-build.md`) | An engine heuristic deriving the head from beat entities (entity lists don't rank the perceiving center — the *Closed Circle* failure is exactly that guess); a POLISH-time per-passage LLM pass (too late to influence grouping, so incompatible beats are already crushed together); per-beat head-switching inside a passage (the corpus's mid-scene head-hopping warning); author-authored heads (doesn't scale past fixtures) | The beats already exist and carry the drama when annotate runs, so the head is dramatic judgment placed where the other per-beat craft signals live — one existing LLM call, schema-pinned so a dangling head is unrepresentable, settled at the freeze; making the passage the rotation unit turns every head-switch into a page-turn (the corpus's "clear structural marker") and gives the invariant a natural gate (G4) |
| A23 | A texture world is an arm of engine-mirrored structural beats (`purpose: texture_world`) parallel to a cap-aligned, consequence-free trunk stretch: the splice copies each twin's *effective* annotations onto the arm beat and stores the twin id in `Beat.mirrors`; invariant I15 holds the contract with a local edge-projection rule (every edge incident to a texture beat projects via `mirrors` onto an existing trunk edge), and cadence diamonds landing inside a mirrored stretch are mirrored into every arm (`insert_cadence_diamond`) so both worlds keep the same choice topology | Computing the arm/trunk correspondence instead of storing it (ambiguous once forks share endpoints — two arms over one stretch, or abutting forks); the false-branch micro_beat fallback for arm banding (starves the parallel world the reader chose — a reader-visible bias); reserving mirrored stretches from cadence (probed: B6 780→1129 at medium-max, the forks displaced ~14 diamonds); full symmetry re-texturing the trunk too (rewriting freeze-settled summaries needs a freeze exception nothing justifies) | `mirrors` is insertion provenance like A14's world suffixes — engine/gate-consumed, never prompt-rendered, and the projection rule turns convergence parity, contiguity, injectivity, and residue-bypass into local edge checks; mirrored cadence realizes the author's "even containing branches" in cosmetic form while keeping density (B6 785 vs 780 with 3 scene-scale worlds planted) |
| A24 | Cosmetic forks are ONE mechanism (author-directed 2026-07-15): k ≥ 2 renderings of a trunk segment — sidetrack/diamond/two-worlds/texture world are parameter choices (segment length, k, empty-rendering presence), renderings are peers (a premise per non-empty rendering *including the trunk segment*, engine-minted cosmetic keyword per non-empty rendering, `Beat.grants_flags` as the beat-layer grant), finalize becomes a fixed-point loop of the one splice (recursion = the next round sees rendering beats as segments), and choice parity between renderings is budget-derived (every walk in the B6 band), not structural — amending A23: the mirrored-cadence twinning retires with the loop, while A23's `mirrors` provenance, edge projection, and no-summary-rewriting stand. Residue arms and dilemma forks stay outside (routed/obligated, not chosen). Contract: `docs/plans/cosmetic-forks.md` | Three shapes as three mechanisms (the shipped state — asymmetric premise made W4 work for half the readers, and the trunk-as-original framing biased minting); grants on choice edges (they don't exist until the passages pass; all flag reasoning is beat-layer, so I10 could never see the grant); structural choice-topology parity via twinned diamonds (re-worded copies; exists only because one-shot finalize decides everything on the pre-fork graph); a distinct flag source for keywords (`FlagSource.COSMETIC`'s definition is exactly this) | The code already said it: `insert_texture_world` makes the trunk "conditionally traversed like a diamond arm" — a texture world IS a diamond whose second arm is pre-existing beats; unifying makes the small-scale two-worlds a parameter (not new machinery), makes recursion a loop property (not mirror-of-mirror machinery), and replaces a parity workaround with the B6 band each walk must hit anyway. The obligation boundary (dilemma = promise, keyword = permission) becomes structural via I16: cosmetic flags requirable only inside converging cosmetic constructs, so "downstream depends on it" is unrepresentable |
| A25 | The unit of viewpoint assignment is the engine-computed *sequence* (maximal choice-free linear run, `grow_sequences` — computed, never stored): a GROW `scheme` pass first resolves `vision.pov_hint` into per-character `pov_head` marks + at most one `interlude_carrier` (roster membership not required), then annotate assigns ONE head per sequence with enum-pinned heads, justified splits, and `""`-headed wide cutaways, and the engine expands to the per-beat `viewpoint` (the stored form, unchanged); I17 holds hand edits to the scheme, B11 reports sequence health (mid-sequence switches, non-coda wides, silent registers, head share). Amends A22's assignment granularity; A22's storage, I14, the collapse cut, and FILL enforcement stand (`docs/plans/pov-sequences.md`) | A stateful sequential prompt ("keep the PoV unless impossible" — sticky bias, order-dependent, a judgment fence every tier samples differently); per-beat assignment with more prose rules (the v1 shape — PREFER RUNS was stated and trusted, and the live medium hopped heads on 58% of linear annotated pairs anyway, I14 shattering 76 of 172 passages as pure POV splits); post-hoc engine smoothing (papers over the defect and overrides dramatic judgment mechanically); a hard split cap (prompt accretion — visibility via B11 instead) | A head-switch inside a choice-free run mints a page-turn the reader pays for, so the run is the natural assignment unit — making the failure unrepresentable beats discouraging it (the same move as engine-assigned fork shapes and I16); rotation planned over the visible sequence list distributes heads better than 240 per-beat decisions; heads held per run: −28% passages on the run-6 counterfactual |
| A26 | Export styles are **data + per-medium furniture**, not a template engine: `export/style.py` owns the contrast-checked ramps, the ratio→placement tables, and large print as a `dataclasses.replace` modifier, while each medium's furniture (running heads, title screens) stays in `gamebook.py` / `html.py`. The accessibility contract is made **mechanical rather than reviewed**: alt text is a required brief field checked at DRESS apply, again at the runtime boundary, again by the print lint, and finally by the Typst compiler itself (`pdf_standards=("ua-1",)`); a ramp that misses a WCAG floor raises `StyleError` and fails the build (04 §7) | A shared cross-medium template layer (Quarto, or a common IR both media render); alt text as an advisory warning; checking contrast in review or CI only; a sixth "large print" style | The two media share a *contract*, not a layout — a print running head and an HTML title screen have no common abstraction worth naming, and the one real overlap (ramp, ratios) is a table. Quarto's Markdown→Typst layer would add a round-trip for markup the exporter already generates, and everything the directions need (running heads reading the current section range, marginal numerals, recto/verso margins, full-bleed pages) is plain Typst. Four checks for alt text is not redundancy but *localization*: Typst's "missing alt text" names neither the passage nor the file, so the earlier checks exist to say where to fix it, and the compiler exists so no path can skip it. Large print varies three numbers over any style's own palette and furniture — a style would fork every future style in two |
