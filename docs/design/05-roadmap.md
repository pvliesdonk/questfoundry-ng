...
| Risk | Mitigation |
|---|---|
| Weak-tier prose quality at scale — a cheap model reaches FILL but exhausts review on the hardest passages | **Largely retired 2026-07-19**: the prose-quality engine (echo check, story-so-far, modulation, review contract) shipped and a weak-tier medium completed DREAM→DRESS gate-clean (`examples/closed-circle-oss`). The corpus-grounded run + author read landed 2026-08-04 (star-swabber): gate-clean, but the read found register nonconformance — **fixed and validated 2026-08-05** (register-conformance stack; validation record and exemplar run: `docs/plans/register-conformance.md` §10) — and the operator loop handles the residual per-passage stalls (roadmap "Later") |
